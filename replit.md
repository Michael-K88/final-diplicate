...
## Recent Changes
- 2026-03-18: Security fix — replaced vulnerable `lodash.set@4.3.2` (prototype pollution CVE) with a local shim (`patches/lodash-set-safe/`) that wraps the patched `lodash@4.17.21+` `set` function. The shim is wired in via npm workspaces so `node_modules/lodash.set` transparently resolves to the safe implementation. `@deriv/deriv-charts` (the dependent) works unchanged.
- 2026-03-12: Batch Trader - added retry logic to buyOne (up to 3 attempts, 300 ms/600 ms back-off). Eliminates "Trade failed" errors from transient API rate-limits or network hiccups — each failed attempt is automatically retried before reporting an error.
